...
const createCenteredIcon = (colorClass, size) =>
  L.divIcon({
    className: "",
...
let refreshIntervalSeconds =
  parseInt(localStorage.getItem("refreshRate")) || 10;
...
function clearDashboard(statusMsg, pos = null, id = null) {
  if (pos) {
    map.setView(pos, 15);
    if (!deviceMarker) {
  ...
  timeEl.className =
    "mt-6 pt-4 border-t border-slate-50 text-[9px] text-red-500 font-medium italic text-center";

  statusEl.innerHTML = `<strong>${statusMsg}</strong>`;
  statusEl.className =
    "text-[10px] uppercase tracking-widest hidden md:block text-red-500";

  const ign = document.getElementById("ignition");
  ign.innerText = "N/A";
  ign.className =
    "px-3 py-1 rounded-full text-[9px] font-black uppercase bg-slate-100 text-slate-400";

  const bars = document
    .getElementById("signal-bars")
    .querySelectorAll(".sig-bar");
...
async function updateLive() {
  ...
  try {
    ...
    if (!data.latitude || parseFloat(data.latitude) === 0) {
      ...
      return;
    } else if (checkDeviceActive(data) === false) {
      const pos = [parseFloat(data.latitude), parseFloat(data.longitude)];
      clearDashboard("Device Inactive", pos, id);
      return;
    }
    ...
    const statusEl = document.getElementById("status-text");
    timeEl.className =
      "mt-6 pt-4 border-t border-slate-50 text-[9px] text-slate-400 font-medium italic text-center";
    statusEl.className =
      "text-[10px] uppercase tracking-widest hidden md:block text-slate-400 font-bold";
    ...
    document.getElementById("odo").innerText =
      odoKm.toLocaleString(undefined, {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      }) + " km";
    document.getElementById("movement").innerText =
      data.movement === 1 ? "Moving" : "Stationary";
    document.getElementById("time").innerText =
      "Last Update: " + (data.timestamp || new Date().toLocaleTimeString());

    const ign = document.getElementById("ignition");
    ign.innerText = data.ignition === 1 ? "IGNITION ON" : "IGNITION OFF";
    ign.className = `px-3 py-1 rounded-full text-[9px] font-black uppercase ${
      data.ignition === 1
    ...
    const gnssStatusMap = {
      ...
      3: "GNSS sleep",
      4: "GNSS ON with fix, invalid data",
    };
    ...
    document.getElementById("operator").innerText =
      OPERATOR_MAP[data.active_gsm_operator] ||
      data.active_gsm_operator ||
      "N/A";

    const v = parseFloat(data.battery_voltage_v) || 0;
    document.getElementById("battery").innerText = v + " V";
    document
      .getElementById("battery-alert")
      .classList.toggle("hidden", v >= 10);
    document.getElementById("status-text").innerText =
      "Signal Received: " + new Date().toLocaleTimeString();
  } catch (e) {
